...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var placeholderViewCenterConstraint: NSLayoutConstraint!
    ...
    func setupMyTextField() {
        ...
        my_TF.leftView = UIView(frame: CGRect(x: 0.0, y: 0.0, width: 15, height: my_TF.frame.height))
    ...
    func textFieldDidStartEditing() {
        NSLayoutConstraint.activate([
            placeholderView.centerYAnchor.constraint(equalTo: my_TF.topAnchor)
            ])
        UIView.animate(withDuration: 0.5) {
            self.view.layoutIfNeeded()
        }
        my_TF.becomeFirstResponder()
        my_TF.layer.borderColor = UIColor(red:0, green:0.73, blue:0.71, alpha:1).cgColor
